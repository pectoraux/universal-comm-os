...
The Android node's signing secret key is stored in the Android Keystore (`KeyStore.getInstance("AndroidKeyStore")`) — never leaves the secure enclave. The public key is exported and shared via the IdentityGraph.
