...
| ARCH-051 | Repository Truth Gate (S0.2.3, Article XVI). A milestone is not COMPLETE until `local HEAD == origin/main HEAD == tested commit SHA` AND the worktree is clean. The agent MUST run the verifiable gate script `scripts/repo-truth-gate.sh` before declaring completion, and MUST include the SHA report in the completion message. CI (GitHub Actions, when enabled) machine-enforces the gate: a workflow on `push` and `pull_request` fails if the worktree is dirty or if the tested SHA does not equal `github.sha`. This was added in response to the S0.2.2 governance failure where the agent reported "all tests pass / milestone complete" while the implementation existed only locally and had not been pushed — GitHub `main` still contained the old 3-state `VerificationState` and returned 404 for the claimed `IdentityLinkStateMachine.ts`. | ACTIVE  |
