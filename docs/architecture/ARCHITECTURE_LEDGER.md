...
| ARCH-046 | Verified contribution measurement (P13): contribution is measured from the delivery tracker's per-node state transitions (OBSERVABLE evidence). True VERIFIABLE evidence would come from RELAY_FORWARD proofs (Ed25519-signed per ARCH-023), but those aren't currently stored centrally. The measurement is a first step; the verifiable-evidence + anti-abuse layer is a future iteration. | ACTIVE  |
| ARCH-047 | Reputation from observable behavior (P13): reputation = delivery success rate (DELIVERED / (DELIVERED + EXPIRED + NO_ROUTE)). Per-node + network-wide. Colored badges: ≥80% emerald, ≥50% amber, <50% red. This is OBSERVABLE reputation, not VERIFIED reputation — a future iteration would cross-verify with RELAY_FORWARD proofs. | ACTIVE  |
| ARCH-048 | No token economy (P13): per master prompt §19, "Do not implement a token/credit economy prematurely. First establish reliable contribution measurement and anti-abuse mechanisms." P13 implements measurement + reputation ONLY — NO tokens, NO credits, NO economy. The evidence note in the UI explicitly states this. | ACTIVE  |

| ARCH-049 | IdentityLink state machine is canonical (S0.2.2). All link state transitions — in-memory IdentityGraph mutations, persisted ChannelVerificationChallenge updates, server-action verifyChannel paths — MUST go through `transition()` in `src/core/identity/IdentityLinkStateMachine.ts`. The state machine is pure (no I/O, no DB writes, no in-memory mutation). Side-effects are the caller's responsibility, but the caller MUST consult `transition()` first so that illegal transitions throw `LinkStateError`. Corrects the S0.2.1 defect where the in-memory `link()` auto-marked entries as `VERIFIED` (Article XIV §1 violation) and `revoke()` deleted entries (Article XIV §6 violation). | ACTIVE  |
| ARCH-050 | In-memory IdentityGraph mirrors the DB state (S0.2.2). Per Article XIV §3 ("Verification state is persisted in the database, not in-memory"), the DB is canonical — the in-memory graph is a cache that mirrors the DB. Production code MUST update the DB first via `verifyChannelChallenge()` / `revokeChannelLink()` in `lib/authorization.ts`; only on DB success does the caller mirror the transition to the in-memory IdentityGraph via `verifyChannel()` / `expireChannel()` / `revoke()`. Cold-start cache misses return false (no throw) — the caller treats this as a reload signal. | ACTIVE  |
| ARCH-051 | Repository Truth Gate (S0.2.3, S0.2.4, Article XVI). A milestone is not COMPLETE until `local HEAD == origin/main HEAD == tested commit SHA` AND the worktree is clean. S0.2.4 extends the gate with TWO modes: (1) **MAIN_MILESTONE** — for declaring a milestone COMPLETE on `main`; requires `local HEAD == origin/main HEAD == tested SHA`. (2) **PR_INTEGRITY** — for verifying a pull request's integrity BEFORE merge; requires `tested SHA == checked-out HEAD (== github.sha in CI)` and does NOT consult `origin/main`. S0.2.4 also makes architecture tests FATAL (was `|| true`), makes security tests a FATAL dedicated subset (`tests/architecture/s0-security.test.ts`), and makes typecheck FATAL (removes `continue-on-error: true` from the CI workflow). The agent MUST run `bash scripts/repo-truth-gate.sh <MODE> <id>` before declaring completion, and MUST include the COMMIT REPORT in the completion message. CI machine-enforces: a workflow on `push: branches: [main]` runs the MAIN_MILESTONE gate; a workflow on `pull_request: branches: [main]` runs the PR_INTEGRITY gate. Branch protection via the GitHub API requires GitHub Pro / public repo (returns `403` on the current private repo; `404` on the branch-protection endpoint when protection is OFF). Compensated by a pre-push hook (`scripts/install-pre-push-hook.sh`) and CI gating. S0.2.2 governance failure motivation: the agent reported "all tests pass / milestone complete" while the implementation existed only locally and the claimed `IdentityLinkStateMachine.ts` returned 404 on GitHub `main` because the commit had not been pushed. | ACTIVE  |
| ARCH-052 | Execution Evidence Manifest (S0.2.5, Article XVII). Closes the last gap in the governance chain: Article XVI proved the tested code is identical to the repository code, but did not prove the test was actually executed. ARCH-052 requires a machine-generated execution evidence manifest (`docs/verification/latest-execution.json`) that records: the milestone ID, the exact `commit_sha` (== HEAD at execution time), the repository URL (credentials stripped), the branch, an ISO-8601 timestamp, the environment (OS / Node / package manager / runtime), the ordered list of commands executed with their `exit_code` and `duration_ms`, and the summary results (tests / architecture / security / typecheck). The manifest is generated by `scripts/generate-execution-evidence.sh` AFTER the Repository Truth Gate has succeeded — the generator RE-EXECUTES the required validation commands (does not trust a prior gate run) and records their actual exit codes. The manifest is verified by `scripts/verify-execution-evidence.sh`: SHA matches HEAD, repository matches origin, all command exit_codes are 0, timestamp is within 30 days, all results contain "passed" with non-zero count and "0 failed". A new `execution-evidence` CI job runs on push and pull_request, generates the manifest, verifies it, and uploads it as a GitHub Actions artifact with 30-day retention. The job does NOT weaken existing gates — it runs AFTER them and adds a new required check. The Article XVI COMMIT REPORT is extended with an `EXECUTION EVIDENCE` block (PATH / SHA / STATUS). A milestone reported COMPLETE without `EXECUTION EVIDENCE STATUS: VALID` is automatically INVALID per Article XVII. | ACTIVE  |
| ARCH-053 | Hardware adapters cannot redefine protocol semantics (P4-DESIGN + S0.2.6, Article XVIII). Hardware adapters (BLE, Wi-Fi Direct, NFC, USB, LoRa, Wi-Fi Aware, etc.) are pluggable transport implementations of the existing `Transport` interface (Article I.2). They MUST NOT introduce new `TransportSendResult` kinds (4 canonical: OK / UNAVAILABLE / NO_PEER / ERROR). They MUST NOT throw across the `Transport` interface boundary (internal exceptions caught and translated to `{ kind: 'ERROR'; reason: string }`). They MUST NOT decrypt or interpret bundle contents (bundles are opaque bytes per THREAT_MODEL §1). They MUST NOT modify any frozen invariant: CommunicationBundle format (Article IV), Universal Identity (Article II), IdentityGraph (Article II), VerificationState machine (Article XIV + XV), Authorization model (Articles XII–XIV), Trust model (Article IX), Delivery state machine (Article VI), Repository Truth Gate (Article XVI), Execution Evidence Gate (Article XVII). They MUST live in `src/transport/<name>/` and import ONLY from `@/core/*` (Article I.2 + `boundaries-strict.test.ts` enforcement). They MAY implement the duck-typed `gossip()` / `onGossip()` side-channel (ARCH-031 — same pattern as `LoopbackTransport`) but ONLY with acceptable payload kinds (PEER_SEEN / PEER_REACHABLE / RESOURCE_REPORT / CAPABILITY_ADVERTISEMENT / FORWARDING_OPPORTUNITY) — forbidden kinds (IDENTITY_ASSERTION / TRUST_ASSERTION / DELIVERY_STATE / AUTHZ_GRANT / BUNDLE_VARIANT / VERIFICATION_ASSERTION) are architecture-control defects (Article XVIII §11). They MAY report resources via the existing `NodeCapabilities.resource` field (ARCH-035). They MAY add link-layer encryption (BLE LE Secure Connections, Wi-Fi Direct WPS) INDEPENDENT of bundle end-to-end encryption (Article IX, Article XVIII §9) — the two-layer separation is a STRUCTURAL property, not a feature; a relay forwards opaque ciphertext without possessing the recipient's X25519 secret key. Transport framing fields (BLE chunk sequence + flag, Wi-Fi length prefix) are EPHEMERAL transport state, NOT bundle semantics — they MUST NOT be serialized into the bundle, persisted in the BundleStore, or alter any protocol field (Article XVIII §10). RELAY_FORWARD proofs (ARCH-023) signed by relays prove forwarding evidence ONLY — NOT sender authority, recipient verification, trust endorsement, authorization, or bundle content endorsement (Article XVIII §12). The StoredBundle contract is ONE protocol-level contract with multiple persistence impls (Prisma/PostgreSQL, Room/SQLite, in-memory) — all impls MUST satisfy cross-impl invariants P1-P7 (bundle identity, dedup identity, idempotent TTL expiry, state-transition-via-DeliveryTracker.transition(), forwarding-proof-only-appends, crash consistency, schema-migration safety) (Article XVIII §13). Violations are automatic architecture-control defects (Article X) — the implementation MUST be reverted and the design MUST be revised. P4 (BLE + Wi-Fi Direct adapters) is the first implementation milestone governed by ARCH-053 / Article XVIII. The P4 architecture design is in `docs/architecture/P4_EDGE_TRANSPORT_ARCHITECTURE.md`. S0.2.6 strengthened ARCH-053 in response to the architecture reviewer's 8-point review of the original P4-DESIGN. | ACTIVE  |
| ARCH-054 | Android runtime lifecycle is canonical (P4.1, Article XVIII §1). The Android Runtime Host (`src/server/android/AndroidRuntimeHost.ts`) owns a 6-state lifecycle: `CREATED → INITIALIZING → HYDRATING → RUNNING → DRAINING → STOPPED`. Forward-only transitions (no skipping, no backward). Illegal transitions return false (not throw — Article XVIII §2). The lifecycle is governed by the canonical `transitionRuntimeLifecycle()` function in `types.ts` which DOES throw `RuntimeLifecycleError` (the host catches and translates to false). Rules: the runtime cannot process protocol work before HYDRATING completes; shutdown (DRAINING) prevents new work; in-flight work reaches a deterministic terminal/recoverable state; restart produces equivalent protocol state (R1). The lifecycle is NOT a second delivery state machine — per-bundle delivery is owned by `DeliveryTracker.transition()` (Article VI). | ACTIVE  |
| ARCH-055 | TransportConformanceSuite is mandatory (P4.1, P4-DESIGN §15). The suite (`src/server/android/conformance/TransportConformanceSuite.ts`) is a reusable test framework proving transport-agnostic invariants for ANY Transport implementation: bundle round-trip, framing boundary, malformed input rejection, duplicate handling, lifecycle cleanup, close/reopen semantics, opaque bundle handling, no identity mutation, no authorization mutation, no delivery-state mutation, resource-reporting isolation. The suite is parameterized by a `TransportFactory` — P4.1 uses `fakeTransportFactory` (FakeTransport); P4.2 will register `bleTransportFactory`; P4.3 will register `wifiDirectTransportFactory`. A conformance suite failure is an Article XVIII §14 architecture-control defect. | ACTIVE  |
| ARCH-056 | Android Keystore boundary (P4.1, Article XVIII §4 + R4). Cryptographic private keys intended for Android-native protection MUST live in the Android Keystore (`KeyStore.getInstance("AndroidKeychain")`), NOT in ordinary application storage. The runtime accesses keys via a `KeystoreAdapter` interface (`sign()`, `getPublicKey()`, `isUnlocked()`). The runtime MUST fail-closed (return null) when the Keystore is locked. The runtime MUST NOT cache the signing secret key in process memory beyond a single signature operation. The runtime MUST NOT introduce substitute cryptographic primitives (Article IX — all crypto via `core/trust/Proof.ts` + `core/trust/CryptoEnvelope.ts`). P4.1 ships a `TestKeystoreAdapter` (test fixture, uses tweetnacl in-memory keypair) — clearly named as test, NOT production. The real Android impl will use AndroidKeychain. | ACTIVE  |
