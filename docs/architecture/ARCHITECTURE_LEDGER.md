...
| ARCH-053 | Hardware adapters cannot redefine protocol semantics (P4-DESIGN, Article XVIII). Hardware adapters (BLE, Wi-Fi Direct, NFC, USB, LoRa, Wi-Fi Aware, etc.) are pluggable transport implementations of the existing `Transport` interface (Article I.2). They MUST NOT introduce new `TransportSendResult` kinds (4 canonical: OK / UNAVAILABLE / NO_PEER / ERROR). They MUST NOT throw across the `Transport` interface boundary (internal exceptions caught and translated to `{ kind: 'ERROR'; reason: string }`). They MUST NOT decrypt or interpret bundle contents (bundles are opaque bytes per THREAT_MODEL §1). They MUST NOT modify any frozen invariant: CommunicationBundle format (Article IV), Universal Identity (Article II), IdentityGraph (Article II), VerificationState machine (Article XIV + XV), Authorization model (Articles XII–XIV), Trust model (Article IX), Delivery state machine (Article VI), Repository Truth Gate (Article XVI), Execution Evidence Gate (Article XVII). They MUST live in `src/transport/<name>/` and import ONLY from `@/core/*` (Article I.2 + `boundaries-strict.test.ts` enforcement). They MAY implement the duck-typed `gossip()` / `onGossip()` side-channel (ARCH-031 — same pattern as `LoopbackTransport`). They MAY report resources via the existing `NodeCapabilities.resource` field (ARCH-035). They MAY add link-layer encryption (BLE LE Secure Connections, Wi-Fi Direct WPS) INDEPENDENT of bundle end-to-end encryption (Article IX). Violations are automatic architecture-control defects (Article X) — the implementation MUST be reverted and the design MUST be revised. P4 (BLE + Wi-Fi Direct adapters) is the first implementation milestone governed by ARCH-053 / Article XVIII. The P4 architecture design is in `docs/architecture/P4_EDGE_TRANSPORT_ARCHITECTURE.md`. | ACTIVE  |
